...
### 按钮溢出修复、存档重置与数值/素材工作台（2026-09-26）

- 修复主菜单按钮文字超出背景框：按钮恢复完整九宫格内容边距，角色/武器/地图信息移到标题右侧，底部只保留地图与操作按钮。
- 新档可玩性：默认解锁免费角色的起始武器（否则 0 金新档无武器）；购买角色附送其起始武器；新增「重置存档」入口（确认框，重置后刷新菜单与解锁服务）。
- 新增「数值/素材工作台」（主菜单 → 设置）：按 13 个分类枚举 `data/` 资源，中文字段名直接取脚本 `@export` 上方的 `##` 注释；支持改数值（即时生效）、更换素材与引用、保存回 `.tres`、放弃修改重载。
- 主要定义脚本补充中文 `@export_group` 分组，Godot Inspector 与工作台同时受益。
- 新增 `docs/数据与素材索引.md`（数据/素材总表与三种修改方式）与 `config_workbench`/`profile_reset` 专项测试。
- 解析、启动、70 项快速冒烟、120 秒 soak 通过；实拍 `art_review/v2_main_menu_preview.png`、`v2_menu_settings_preview.png`、`v2_workbench_preview.png`。

### 菜单按钮修复与剩余美术素材接入（2026-09-26）

- 修复「退出游戏」加入后菜单/暂停按钮被压扁：按钮样式保留上下九宫格纹理边距，仅收窄左右内容边距，按钮高度恢复九宫格所需尺寸；主菜单地图区由竖排改横排，释放卡片列表高度并保证 1280 宽内不溢出。
- 接入 11 个通用属性升级图标（迅捷施法、弹数、弹体大小、弹速、魔法回响、额外弹、生命上限、暖心治疗、闪避、免疫、贯穿）；修正 `wire_combat_visuals.py` 把 `icon` 插到 `script` 之前的问题，并在 `v2_art_validation` 增加“所有升级必须有新版图标”检查。
- 主菜单三张卡片改用 `menu_card_base` 卡框、分区标题改用 `menu_tab` 标签底；升级三选一改为竖版卡面并应用 `ui_upgrade_card`（卡面 2:3、图标置顶，含刷新按钮）。
- 解析、启动、68 项快速冒烟、120 秒 soak 通过；实拍 `art_review/v2_main_menu_preview.png`、`v2_settings_preview.png`、`v2_upgrade_panel_preview.png`。

### 十分钟终局与后期升级节奏（2026-09-25）

- 默认单局与 Boss 登场时间由 5 分钟延长至 10 分钟，HUD 和目标提示跟随配置显示。
- 单局过半后，基础强化卡权重变为原后期前的 25%，通用卡变为 50%；基础强化保底概率降至 50%，避免尚未满级的武器失去质变路径。已满足条件的质变仍保底一张，额外卡位中的质变权重再乘 2.5。阈值与倍率均可在 `UpgradeSystem` 导出配置调整。
- 新增十分钟与前后期升级概率专项测试，并同步更新 Boss、完整单局与计时回归。

### 菜单与单局平衡修订（2026-09-25）

- 升级三选一改为按 `weight` 加权无放回抽取；可用质变卡保底一张，未满足质变时优先已持有装备的基础强化；未持有对应装备的 Buff 不再进入候选。
- 正式武器目录与单局升级池只保留设计稿中的弓、法杖、剑；默认法师改以法杖开局。旧武器资源保留供兼容测试使用，不再从正式菜单或升级池获取。
- 主菜单改为角色、武器、永久成长三分区，并新增明确标为测试用途的一键解锁按钮：解锁目录内容、升满永久强化、提供刷新次数，不扣金币且存档失败回滚。
- 提高四阶段普通敌人的刷新批次与存活上限；最终 Boss 生命提高，新增三向远程弹幕，登场后普通敌人继续刷新。
- 新增平衡专项冒烟测试与旧武器测试专用场景，更新现行配置的回归断言。真实窗口五分钟主观手感与长期平衡仍需人工验收。
...
### T29 武器/角色解锁与购买（2026-09-25）

- 角色/武器新增 `unlock_cost`（0 为默认解锁）；`Profile` 新增 try_unlock 接口，`UnlockService` 作为唯一交易入口，写档失败回滚内存档案（D26）。
- 主菜单显示“未解锁 价格/已解锁”，点击锁定项即购买并刷新候选池；入口与菜单都只接受已解锁内容，进行中的一局不受购买影响。
- 新增 `unlock_shop_smoke_test.gd`；解析、启动与 57 项快速冒烟、120 秒 soak 通过。

### T30 永久强化与开局快照（2026-09-25）

- 新增 `PermanentUpgradeDefinition`/`PermanentUpgradeCatalog` 与默认目录（移速/伤害/恢复/吸血/防御，各 5 级、价格递增），全部可配（D27、D11）。
- `UnlockService` 新增永久强化购买（上限/余额校验、写档失败回滚）；主菜单新增永久强化区（等级/价格/购买）。
...
### T33 新敌人与地图（2026-09-25）

- 新增远程敌人「月影投手」（保持距离、发射弹体）与精英近战「石甲兽」（高血量、更快、接触伤害），复用 `EnemyActor` 与公共组件（D30）。
- `EnemyDefinition` 新增攻击方式/射程/冷却/保持距离/弹体；`ProjectileDefinition` 新增命中层与着色（敌人弹体打玩家层、红色区分）；两者混入第 2–4 段难度。
- 地图数据驱动：`ArenaDefinition` + `Arena.configure` + 「月蚀荒原」（背景素材待画，配色占位）。
- 新增 `enemy_map_smoke_test.gd`；解析、启动与 61 项快速冒烟、120 秒 soak 通过。

### T33–T35 收口（2026-09-25）

- T34 界面收口：主菜单应用面板/按钮样式并改左右两栏（修复永久强化区导致的溢出），局内新增金币图标与数量；主菜单图形实拍复核。
- T35 完成自动验收（解析、300 帧、61 项快速冒烟、120 秒 soak）；真实窗口五分钟人工游玩与主观/长期验收无法在本环境执行，标记未完成。

### 素材接入（第二批）与科技激光（2026-09-25）

- 接入 B02 射手、月影投手、石甲兽、月蚀荒原背景、铁剑与斩击弧光、喷气/导弹尾焰、主菜单整套、科技徽记、敌方弹体；新敌人/射手改为独立场景，地图/武器/弹体/套装指向新素材。
- 科技激光按用户指定实现为“同时获得、交替攻击”：发射器奇数发 3 枚爆炸导弹、偶数发 1 道高速穿透激光（D31）。
- 主菜单使用新九宫格面板与三态按钮、新背景；HUD 显示当前地图名与套装徽记。
- 修复 `ending_restart` 因随机首卡为闪避/免疫导致的偶发不死（仅测试）；解析、启动与 61 项快速冒烟、120 秒 soak 通过。

### 科技激光素材接入（2026-09-25）

- 接入激光射束、命中光斑与口闪素材；`ProjectileDefinition` 新增命中/开火特效字段，`ProjectileBase` 命中与开火时生成一次性特效。
- `tech_laser.tres` 改用激光射束贴图并接入命中/口闪；科技发射器“导弹↔激光”交替攻击视觉完整。
- 解析、启动与 61 项快速冒烟、120 秒 soak 通过；实拍 `art_review/v2_new_content_preview.png`。

### T35 完整一局自动验收（2026-09-25）

- 新增 `full_run_content_smoke_test.gd`：真实单局内验证新敌人远程弹体、五分钟 Boss、胜利结算一次、结算清理与重开隔离。
- 自动验收：解析、启动、62 项快速冒烟、120 秒 soak 全部通过；真实窗口五分钟人工游玩与主观/长期验收仍待用户在目标机执行，标记未完成。

### 装备显示/设置/金币兜底/地图选择（2026-09-25，用户追加）

- 装备栏显示图标与名称，质变后显示“基础名·分支名”并换用质变图标（D32）；`EquipmentProgress` 新增 `branch_display_name`/`branch_icon`。
- 局内 Esc 打开设置面板（声音开关 + 返回主菜单），主菜单也有设置；菜单静音偏好在开局应用到单局。
- 升级池无可用卡时给出金币兜底卡 `COIN_REWARD`（`UpgradeSystem.fallback_upgrades`），满级后不再空转。
- `ContentCatalog` 新增 `maps`，`RunLoadout` 新增 `map_id`，主菜单可选择月光庭院/月蚀荒原并在单局应用。
- 修复既有 `icon` 属性写在 `script` 之前导致防具与升级卡图标被忽略的问题；主菜单底部控件收窄避免溢出。
- 新增 4 个专项测试；解析、启动与 68 项快速冒烟、120 秒 soak 通过。

### 升级卡图标与排版修复（2026-09-25）

- 修复升级卡带图标时排版错乱：卡面改为自绘布局（图标 + 分类标题 + 描述），不再用 `Button.icon` 与多行文本混排。
- 为 46 个升级卡补上对应图标（获取卡用所获武器/防具图标、基础/质变/专属用对应武器分支或成长图标、金币卡用金币图标）。
