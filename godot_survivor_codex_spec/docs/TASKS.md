...
## 用户追加：按钮溢出修复、存档重置与数值/素材工作台（2026-09-26）

- [x] 修复主菜单按钮文字超出背景框（恢复完整边距，底部信息移到标题右侧）
- [x] 新增「重置存档」（带确认）；修正新档起始武器解锁；购买角色附送起始武器
- [x] 新增「数值/素材工作台」：13 类资源、中文字段名、改数值即时生效、更换素材/引用、保存回 `.tres`
- [x] 主要定义脚本补中文导出分组；新增数据/素材索引文档
- [x] 解析、启动、70 项快速冒烟、120 秒 soak 通过；实拍主菜单/主菜单设置/工作台
...
- [x] 生成 19 个 SpriteFrames 并接入弹体、武器、爆炸、火坑、冻结、刺圈、金币
- [x] 防具与升级写入图标，升级卡显示图标
- [x] `WeaponDefinition.visual_scale` 可配置并调整弹体显示比例
- [x] 图形模式实拍与总览截图复核，解析/启动/55 项快速冒烟（+120 秒 soak）通过
- [ ] 第四批素材（远程射手、剑与斩击、主菜单版式、敌人/地图）待玩法定稿
- [ ] C08 激光素材与实现（后续可配置分支）

素材交付说明见 `assets/v2_dark_comic/新增素材交付清单.md`，接入证据见 `docs/实施蓝图/执行记录.md` 的“素材接入”条目与 `art_review/`。
...
- [x] D19 首版：固定减伤进统一管线，完全免伤受 CombatRules 严格上限
- [x] `KnightArmorDefinition` 可配固定减伤与免疫概率
- [x] 坚壁/圣佑专属升级，与反伤刺甲互斥
- [x] 0%/100% 免伤边界、上限钳制、免伤不反伤验证
- [x] 专项测试 + 50 项快速回归、120 秒 soak 通过
- [ ] C06/E04 素材与 GUI 人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T22 条目。

### T23 狂战盔甲

- [x] D20 首版：失血绕过防御且默认不致死，吸血动态加成，一次免死进统一管线
- [x] `BerserkArmorDefinition` + `BerserkDrainComponent` 周期失血
- [x] 忍痛/嗜血/不屈三档专属升级，与反伤/骑士互斥
- [x] 半血吸血、免死一次性（含 DoT）、重开归零验证
- [x] 专项测试 + 51 项快速回归、120 秒 soak 通过
- [ ] C06/E04 素材与 GUI 人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T23 条目。

### T24 头盔、手套与科技分支

- [x] D21 首版：头盔小防御无惩罚、手套防御+冷却+射程、科技单件每秒经验与宝石倍率
- [x] `ArmorDefinition` 新增冷却/射程成长，玩家汇总并由武器读取
- [x] 基础头盔/手套与获取/强化卡、目录接入
- [x] 科技战甲/头盔/手套质变，经验走统一入口、宝石单次结算、暂停不计时
- [x] 专项测试 + 52 项快速回归、120 秒 soak 通过
- [ ] C07/D05 图标与 GUI 人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T24 条目。

### T25 科技三件套与发射器

- [x] D22 首版：三件都需科技质变，状态只在切换时应用/撤销一次
- [x] `TechSetDefinition` + 可撤销高移速 + 飞行仅视觉
- [x] 数据驱动科技发射器，随机目标取自 TargetingService，导弹带爆炸
- [x] 用户确认：激光作为后续可配置分支，本任务不实现
- [x] 专项测试 + 53 项快速回归、120 秒 soak 通过
- [ ] C08/E05/D01 素材与 GUI 人工验收
- [x] 补：激光实现（用户指定同时获得、交替攻击，D31）

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T25 条目。

### T26 金币掉落与一次性结算

- [x] D23 首版：掉率/数量/结算公式全在 `EconomyRules`
- [x] 金币拾取物（原子一次）与经验宝石并存，拾取信号
- [x] 结算区分已拾取与击退奖励，`GameResult` 一次性生成
- [x] 重开余额归零、地面金币清空、零击杀为 0
- [x] 专项测试 + 54 项快速回归、120 秒 soak 通过
- [ ] D03 金币素材与 GUI 人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T26 条目。

### T27 版本化存档与安全读写

- [x] D24 首版：`Profile` 只存标量/稳定 ID，读档校验/迁移/丢未知 ID
- [x] `ProfileStore` 原子写入（临时文件+校验+备份+替换），失败保留旧档
- [x] `GameEntry` 加载档案并在结算累加金币，菜单显示存档状态
- [x] 损坏/缺字段/旧版本/写入失败/连续结算/进程重启用例
- [x] 专项测试 + 55 项快速回归、120 秒 soak 通过
- [ ] 正式菜单进度页与 GUI 人工验收（T29/T30）

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T27 条目。

### T28 升级选项刷新次数

- [x] D25 首版：每局刷新 = 基础 + 档案加成，消耗一次并优先换卡
- [x] `UpgradeSystem` 刷新接口与 `LevelUpPanel` 刷新按钮（禁用态）
- [x] 跨升级保留、空池不死锁、重开归零
- [x] 专项测试 + 56 项快速回归、120 秒 soak 通过
- [ ] 刷新按钮 GUI 点击人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T28 条目。

### T29 武器/角色解锁与购买

- [x] D26 首版：价格写 Resource，`UnlockService` 唯一交易入口，失败回滚
- [x] 菜单锁定/价格显示与点击购买，购买后刷新候选池
- [x] 开局只接受已解锁内容，进行中的一局不被修改
- [x] 专项测试 + 57 项快速回归、120 秒 soak 通过
- [ ] 正式商店版式与 GUI 点击人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T29 条目。

### T30 永久强化与开局快照

- [x] D27 首版：属性/增量/上限/价格曲线全在 Resource
- [x] `UnlockService` 购买永久强化，上限/余额校验与写档回滚
- [x] 开局快照写入玩家（移速/伤害/恢复/吸血/防御），运行中不读档案
- [x] 购买不改变进行中的一局，新局生效
- [x] 专项测试 + 58 项快速回归、120 秒 soak 通过
- [ ] 正式商店版式与 GUI 人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T30 条目。

### T31 远程射手角色

- [x] D28 首版：射手复用 PlayerActor，被动按武器标签生效（远程 +10%）
- [x] `CharacterDefinition` 被动字段与 `WeaponDefinition.tags`，晚获取继承
- [x] 射手默认锁定、金币解锁、默认解锁仅价格 0 内容
- [x] 专项测试 + 59 项快速回归、120 秒 soak 通过
- [ ] B02 立绘/头像与 GUI 可辨认人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T31 条目。

### T32 基础剑与近战扇形

- [x] D29 首版：`MELEE_FAN` 攻击形态，不发射弹体
- [x] 扇形内每敌一次、超距/扇外不计、冷却/暴击复用管线
- [x] 铁剑 Resource + 获取卡 + 基础强化，接入目录与升级池
- [x] 专项测试 + 60 项快速回归、120 秒 soak 通过
- [ ] C05 斩击素材与 GUI 人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T32 条目。

### T33 新敌人与地图（首版，用户授权自行设计）

- [x] D30 首版：远程敌人保持距离射击；弹体命中层与着色区分敌我
- [x] 远程「月影投手」与精英近战「石甲兽」，混入第 2–4 段难度
- [x] 数据驱动地图 `ArenaDefinition` + 「月蚀荒原」（配色占位）
- [x] 专项测试 + 61 项快速回归、120 秒 soak 通过
- [ ] 新敌人/新地图素材与 GUI 人工验收

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T33 条目。

### T34 现有素材债与界面收口

- [x] 主菜单应用面板/按钮样式，改为左右两栏，修复内容溢出
- [x] 局内金币图标与数量显示
- [x] 战斗素材接入状态写入美术素材清单
- [x] 主菜单图形模式实拍复核；60 项快速回归通过
- [ ] C05/B02/F03/C08 素材待外部交付

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T34 条目。

### T35 完整游玩、性能与平衡验收

- [x] 解析、300 帧启动、62 项快速冒烟、120 秒 soak 通过
- [x] 完整一局端到端自动冒烟（新敌人 + 五分钟 Boss + 胜利结算 + 重开）
- [ ] 真实窗口五分钟人工游玩（本环境无法执行，标记未完成）
- [ ] 主观手感/音效、macOS 长稳、长期平衡、双角色可辨认、新敌人/激光强度

验证记录：见 `docs/实施蓝图/执行记录.md` 的 T35 条目。

### T35 后续平衡与菜单修订（2026-09-25）

- [x] 质变出现率提高：达到前置条件时三选一保底一张；基础强化优先，其他选项按配置权重抽取
- [x] 未持有武器/装备的对应 Buff 不进入升级候选；正式武器限定弓、法杖、剑
- [x] 主菜单三分区重排与测试用一键解锁（角色、武器、永久强化、刷新次数）
- [x] 各阶段基础怪刷新密度与上限提高；Boss 生命提升、增加三向远程攻击，登场后小怪继续刷新
- [x] Godot 解析、启动、专项及完整快速冒烟测试；1280×720 菜单截图检查
- [ ] 真实窗口连续五分钟游玩与长期平衡/性能主观验收（当前仅完成自动化与菜单画面检查）

验证：`Godot --headless --path . --editor --quit`、`--headless --path . --quit-after 120`、`tests/smoke/balance_revision_smoke_test.gd` 与 `tests/smoke/*_smoke_test.gd`；旧武器低层机制测试使用 `tests/fixtures/legacy_main.tscn`，不影响正式内容池。
