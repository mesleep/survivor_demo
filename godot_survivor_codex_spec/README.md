...
使用 Godot 4.7.1 和 GDScript 分阶段开发的 2D 俯视角 Survivor-like 最小 Demo。四个开发阶段均已实现：玩家移动并自动攻击，击杀后拾取经验和三选一成长，敌人随时间增强，10 分钟后迎战 Boss，最终进入胜利或失败结算并可重新开始。
...
开局约每 0.7 秒生成一批基础敌人，随后按时间提高密度和强度，2 分 30 秒后加入快速敌人；10 分钟时生成 Boss，普通敌人仍会持续刷新。5 分钟后升级池降低基础与通用 Buff 权重，提高质变权重。HUD 显示生命、等级、经验和倒计时；升级和结算时游戏暂停。
...
- 当前默认法师以蓄力法杖开局；正式候选武器为弓、法杖、剑。星光魔杖、飞叶刃、回旋骨棒、环绕铃铛等旧资源仅供历史兼容测试，不在正式升级池中出现。
